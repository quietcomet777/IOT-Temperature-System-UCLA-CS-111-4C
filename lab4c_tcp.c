...
int main(int argc, char* argv[])
{
	...
	if(optind +1 < argc)  // these two conditionals deal with the portNum which is non switch
	{
	...
	int socketFd;
    struct sockaddr_in serv_addr; // here we set up a socket, just like in lab 1b
    ...
    write(socketFd, idString, strlen(idString));
	...
	stdinPoll[0].fd = socketFd; // changed fd to socket for polling 
	...
	while(run_flag)
	{
		...
		if((rawTime >= NextReport) & reportOn )
		{
			...
			int k;
			int currentChars = 0;
			// here we store all all the chars into a single buf for a  single write
			for(k = 11;  k < 20;k++)
			{
			...
			write(socketFd,serverWriteBuf,currentChars);
		...
		{
			sysFail = read(socketFd,&buf,1024);
			int k;
				...
				if(buf[k] == '\n')
				{
					...
					else
					{
						...
						if(!strcmp(PossibleCommand1,"LOG "))
						{
							...
							write(socketFd,nextbuf,strlen(nextbuf));
							if(logFd > -1)
							{
	...
	write(socketFd,lastBuf,currentChars+9);
